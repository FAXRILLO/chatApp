...
const userCtrl = {
  ...
  getUser: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        ...
        return res
          .status(200)
          .json({ message: "Found user", user: otherDetails });
      }
  ...
  deleteUser: async (req, res) => {
    ...
    try {
      if (id === req.user._id || req.userIsAdmin) {
        ...
        if (deletedUser) {
          ...
          if (deletedUser.coverPicture) {
            fs.unlinkSync(path.join(uploadsDir, deletedUser.coverPicture), (err) => {
              if (err) {
                return res.status(503).send({ message: err.message });
              }
            });
      ...
      res.status(405).json({
        message: "Acess Denied. You can delete only your own account!",
      });
    } catch (error) {
      console.log(error);
      res.status(503).json({ message: error.message });
        ...
        if (req.files) {
          ...
          if (req.files.coverPicture) {
            ...
            if (user.coverPicture) {
              fs.unlinkSync(path.join(uploadsDir, user.coverPicture), (err) => {
                if (err) {
                  return res.status(503).send({ message: err.message });
                }
              });
